...
parser.add_argument('--file',type=str,default="./insts/circ/circ4.txt",help='算例文件')
parser.add_argument('--maxP',type=int,default=1250,help='最大冷却阶段次数')
parser.add_argument('--maxC',type=int,default=1000,help='最大连续拒绝次数')
parser.add_argument('--maxR',type=int,default=1,help='最大重启次数')
parser.add_argument('--T',type=float,default=400.,help='初始温度')
parser.add_argument('--beta',type=float,default=0.999,help='降温系数')
parser.add_argument('--weight',type=float,default=4000,help='惩罚权重')
parser.add_argument('--teta',type=float,default=1.04,help='权重调节因子')
args=parser.parse_args()

# Variables for Simulated Annealing, set manually
file = args.file # Input file
maxP = args.maxP # 最大冷却阶段次数 当连续拒绝改进次数达到maxC时进入下一冷却阶段
maxC = args.maxC # 最大连续拒绝次数 每个冷却阶段内允许的最大连续拒绝移动次数，防止局部停滞
maxR = args.maxR # 最大重启次数 当连续maxR次重启后仍无改进时终止算法
T = args.T # 初始温度 决定初始阶段的探索能力，高温允许更多劣质解接受
beta = args.beta # 降温系数 控制温度衰减速度：T = T * beta
weight = args.weight # 惩罚权重w0 用于将约束违规转换为成本惩罚的调节因子
teta = args.teta # 权重调节因子delta=theta 动态调整惩罚权重的战略振荡参数：
...
def simulated_annealing(maxP,maxC,maxR,T,beta,weight,teta,start_time):
    cost_history = []
    ...
    while reheat <= maxR:
        ...
        while phase <= maxP:
            ...
            while counter <= maxC:
                ...
                if accept == 1:
                    ...
                    if nbf < bestFeasible or nbi < bestInfeasible:
                        ...
                        bestTime = time.perf_counter() - start_time  # Get the time elapsed since the start of the program
                        print(f"Cost = {nbf:.4f} at {bestTime:.2f} seconds")     # Print the cost and the time
            ...
            print(f"Cooling {phase} T = {T:.4f} Cost = {nbf:.4f} at {bestTime:.2f} seconds")
    ...
    return bestSolution, bestFeasible, violationsS, cost_history
...
if __name__ == "__main__":
    ...
    # 调用函数并接收返回值
    best_solution, best_cost, violations, cost_history = simulated_annealing(maxP,maxC,maxR,T,beta,weight,teta,start_time)
    # 输出结果
    print("\nOptimal Solution:")
    print(best_solution)
    ...
    plt.grid(True)
    plt.savefig('SA_cost.png',dpi=600)